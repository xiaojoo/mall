...
@WebMvcTest(
    controllers = MemberLevelController.class,
    properties = "spring.session.store-type=none"
)
class MemberLevelControllerTest {
    ...
    void testList() throws Exception {
        ...
        when(memberLevelService.queryPage(any())).thenReturn(pageUtils);

        mockMvc.perform(get("/api/member/memberlevel/list")
                        .param("page", "1")
                        .param("limit", "10"))
    ...
    void testInfo() throws Exception {
        ...
        entity.setId(1L);
        entity.setName("VIP");
        when(memberLevelService.getById(1L)).thenReturn(entity);

        mockMvc.perform(get("/api/member/memberlevel/info/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));
    }
    ...
    void testSave() throws Exception {
        ...
        mockMvc.perform(post("/api/member/memberlevel/save")
    ...
    void testUpdate() throws Exception {
        ...
        mockMvc.perform(post("/api/member/memberlevel/update")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(entity)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));
    ...
    void testDelete() throws Exception {
        mockMvc.perform(post("/api/member/memberlevel/delete")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("[1,2]"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));
