...
@WebMvcTest(MemberLoginLogController.class)
class MemberLoginLogControllerTest {
    ...
    void testList() throws Exception {
        ...
        when(memberLoginLogService.queryPage(any())).thenReturn(pageUtils);

        mockMvc.perform(get("/api/member/memberloginlog/list")
                        .param("page", "1")
                        .param("limit", "10"))
    ...
    void testInfo() throws Exception {
        ...
        entity.setId(1L);
        when(memberLoginLogService.getById(1L)).thenReturn(entity);

        mockMvc.perform(get("/api/member/memberloginlog/info/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));
    }
    ...
    void testSave() throws Exception {
        ...
        mockMvc.perform(post("/api/member/memberloginlog/save")
    ...
    void testUpdate() throws Exception {
        ...
        mockMvc.perform(post("/api/member/memberloginlog/update")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(entity)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));
    ...
    void testDelete() throws Exception {
        mockMvc.perform(post("/api/member/memberloginlog/delete")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("[1,2]"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));
