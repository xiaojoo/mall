-- ============================================================
-- mall_pms 数据库全量脚本（合并版：表结构 mall_pms + 商品分类种子数据 pms_catelog）
-- 执行数据库：mall_pms（mall-product 所在库）
-- ============================================================
...
(
    ...
    member_id            bigint comment '会员id（JWT 解析，用于每人每商品最多评价 2 条）',
    parent_id            bigint comment '父评论id（0-顶级评论；回复时指向被回复评论id）',
...
/*==============================================================*/
/* Table: pms_spu_comment_report                                */
/*==============================================================*/
create table pms_spu_comment_report
(
    id                   bigint not null auto_increment comment 'id',
    comment_id           bigint comment '被举报评论id',
    spu_id               bigint comment '商品id',
    spu_name             varchar(255) comment '商品名字',
    member_nick_name     varchar(255) comment '举报人昵称',
    reason               varchar(255) comment '举报原因',
    create_time          datetime comment '创建时间',
    status               tinyint(1) comment '处理状态[0-待处理，1-已处理]',
    primary key (id)
);

alter table pms_spu_comment_report comment '商品评论举报';
